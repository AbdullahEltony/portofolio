...
const Nav = () => {
  ...
  return <nav className='flex flex-col items-center xl:justify-center gap-y-4 fixed h-max bottom-0 mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen'>
    ...
    <div className='flex w-full xl:flex-col justify-between
     xl:justify-center items-center gap-y-10 px-4 md:px-40
      xl:px-0 h-[51px] xl:h-max py-8 backdrop-blur-sm bg-white/10
      ...
      {navData.map((link,index)=>{
        return <Link className={`${link.path ===pathName && 'text-accent'} relative flex items-center group hover:text-accent transition-all duration-300`} key={index} href={link.path}>
            ...
            <div className='absolute pr-14 right-0 hidden xl:group-hover:flex'>
              <div className='bg-white text-primary p-[6px] relative flex rounded-[3px]'>
                <div className='text-[12px] leading-none semibold capitalize'>{link.name}</div>
            ...
            <div className='text-[22px] sm:text-2xl' >{link.icon}</div>
          </Link>
      })}
    </div>
  </nav>;
